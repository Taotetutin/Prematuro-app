    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
      <div className="container mx-auto px-4 py-4 sm:py-8">
        <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="bg-gradient-to-r from-blue-400 to-blue-600 p-4 sm:p-6">
            <div className="flex items-center justify-center space-x-3">
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-white">Calculadora de Riesgo de Parto Prematuro</h1>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-8">
            ...
            <button
              ...
              className="mt-6 w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white py-3 px-6 rounded-lg font-semibold hover:from-blue-600 hover:to-blue-700 transition duration-300 ease-in-out active:transform active:scale-95"
            >
              Calcular Riesgo
            </button>
              ...
              <div className="mt-6 p-4 sm:p-6 bg-blue-50 rounded-xl">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className={`p-3 sm:p-4 rounded-lg shadow-sm ${results.oneWeek >= 5 ? 'bg-red-50' : 'bg-white'}`}>
                      <p className="text-sm text-gray-600">Riesgo a 1 semana</p>
                      <p className={`text-xl sm:text-2xl font-bold ${results.oneWeek >= 5 ? 'text-red-600' : 'text-blue-600'}`}>
                        {results.oneWeek.toFixed(1)}%
                      </p>
                    </div>
                    <div className={`p-3 sm:p-4 rounded-lg shadow-sm ${results.twoWeeks >= 10 ? 'bg-red-50' : 'bg-white'}`}>
                      <p className="text-sm text-gray-600">Riesgo a 2 semanas</p>
                      <p className={`text-xl sm:text-2xl font-bold ${results.twoWeeks >= 10 ? 'text-red-600' : 'text-blue-600'}`}>
                        {results.twoWeeks.toFixed(1)}%
                      </p>
                    </div>
                    <div className={`p-3 sm:p-4 rounded-lg shadow-sm ${results.fourWeeks >= 15 ? 'bg-red-50' : 'bg-white'}`}>
                      <p className="text-sm text-gray-600">Riesgo a 4 semanas</p>
                      <p className={`text-xl sm:text-2xl font-bold ${results.fourWeeks >= 15 ? 'text-red-600' : 'text-blue-600'}`}>
                      ...
                      <div className="space-y-3 mt-4">
                        {getRecommendations(formData.gestationalWeeks, formData.uterineDynamics === 'yes').map((rec, index) => (
                          <div key={index} className="flex items-start space-x-2">
                            {rec.icon}
                            <p className="text-red-700 text-sm sm:text-base">{rec.text}</p>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <footer className="mt-6 text-center text-gray-600 text-sm">
          ...
          <p className="mt-1">Esta herramienta es solo una guía. La decisión final debe ser tomada por un profesional de la salud.</p>
        </footer>
      </div>
    </div>
  );
